...
### Does Autisim Med/Treatment affect Developmental Delay, Speech Disorder, Learning Disorder, or Intellectual Disability Severity?

df['Autism_Med'] = df['autismmed'].apply(lambda x: True if x == 1 else False)
df['Autism_Treat'] = df['autismtreat'].apply(lambda x: True if x == 1 else False)
df = df.rename(columns={"k2q35b": "Autism_Currently", "k2q35c": "Autism_Severity", "k2q36c": "DD_Severity", "k2q37c": "SD_Severity", "k2q30c": "LD_Severity", "k2q60c": "ID_Severity"})

training = df[["Autism_Currently", "Autism_Med", "Autism_Treat", "Autism_Severity", "DD_Severity", "SD_Severity", "LD_Severity", "ID_Severity"]]
...
LD_Severity;
ID_Severity;
Autism_Currently -> Autism_Med; Autism_Currently -> Autism_Treat; Autism_Currently -> Autism_Severity; 
Autism_Med -> Autism_Severity; Autism_Treat -> Autism_Severity; 
Autism_Severity -> DD_Severity; 
DD_Severity -> SD_Severity; DD_Severity -> LD_Severity; DD_Severity -> ID_Severity; 
}
"""

res_list = []

### Developmental Delay Models
model= CausalModel(
        data = training,
        graph=causal_graph.replace("\n", " "),
        treatment='Autism_Med',
        outcome='DD_Severity')
#model.view_model()
...
res = {"Treatment" : str(model._treatment), "Outcome" : str(model._outcome), "Causal Estimate" : estimate.value}
res_list.append(res)
...
### Intellectual Disability Models
model= CausalModel(
        data = training,
        graph=causal_graph.replace("\n", " "),
        treatment='Autism_Med',
...
def graph_res(res_lst):
        ...
        plt.title("Autism Cause & Effect Estimations")
        plt.show()
