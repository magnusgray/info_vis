...
### Does ADD Med/Treatment affect Behavioral Problem Severity?

df['ADD_Med'] = df['k2q31d'].apply(lambda x: True if x == 1 else False)
df['ADD_Treat'] = df['addtreat'].apply(lambda x: True if x == 1 else False)
df = df.rename(columns={'k2q31b': 'ADD_Currently', 'k2q31c': 'ADD_Severity', 'k2q34c' : "BP_Severity"})

training = df[["ADD_Currently", "ADD_Severity", "ADD_Med", "ADD_Treat", "BP_Severity"]]
...
ADD_Severity;
BP_Severity;
ADD_Currently -> ADD_Severity; ADD_Currently -> ADD_Med; ADD_Currently -> ADD_Treat; 
ADD_Med -> ADD_Severity; ADD_Treat -> ADD_Severity; 
ADD_Severity -> BP_Severity; 
}
"""
...
def graph_res(res_lst):
        ...
        plt.title("ADD Cause & Effect Estimations")
        plt.show()
